...
export class BuscarPage implements OnDestroy, OnInit {
 ...
 ObtenerPelicula(event){
  ...
  if(this.buscar=="" || this.buscar==null){
      this.alerta('Ingrese el título de una pelicula', 'danger');
  }else{

   this.peliculas.BuscarPelicula(this.buscar, this.ContadorPagina).then(data => {
    for (let index = 0; index < data['results'].length; index++) {
      this.lista.push(data['results'][index]);   
    }

    if(this.lista.length==0||this.lista.length==null){
       this.alerta('Pelicula no encontrada', 'danger');
      }
